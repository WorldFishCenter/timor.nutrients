...
Timor.nutrients::nutrients_table %>% 
  ...
  ggplot2::labs(y = "", x = "Matched RNI from 100g portion")
```
...
parallel_plot <- 
  ...
  dplyr::mutate(nutrient_sum = sum(concentration_g),
...
parallel_plot$y <- factor(parallel_plot$y, levels = c(
  "protein", "omega3", "calcium", "iron", "vitaminA", "zinc", "selenium",
...
parallel_plot %>%
  ...
  ggforce::geom_parallel_sets_axes(axis.width = 0.25, fill = "grey90") +
  ggforce::geom_parallel_sets_labels(colour = "black", angle = 0, size = 3.5) +
