...
tot_prod <- 
  timor.nutrients::map_data |> 
    ...
    dplyr::rename(annual_catch_tot = annual_catch) |> 
    dplyr::rename(WRA_tot = WRA) |> 
    dplyr::mutate(region = ifelse(region == "Liquiça", "Liquica", region))


region_nutr <-
  timor.nutrients::map_data_adj %>%
  dplyr::rename(region = area) %>%
  dplyr::filter(!region == "National") %>%
  dplyr::mutate(region = ifelse(region == "Liquiça", "Liquica", region)) %>%
  dplyr::right_join(bonds, by = c("region")) |> 
  dplyr::left_join(tot_prod, by = "region") |> 
  dplyr::select(region, 
  annual_catch_tot, 
  WRA,
  geometry)
...
p1 <- 
  ...
  geom_sf(aes(geometry = geometry, fill = annual_catch_tot),
...
p2 <- 
  ...
  annotate(
    "text",
    x = 126, y = -8.2, label = "Atauro 747%", size = 3, color = "firebrick", fontface = "bold"
  ) +
  # annotate(
